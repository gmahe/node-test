...
function resolveAndFetch(ctx, wantedDependency, options) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const id = pkgId;
            core_loggers_1.progressLogger.debug({ status: 'resolved', pkgId: id, pkg: options.loggedPkg });
            ...
            if (options.skipFetch && pkg) {
                return {
                    body: {
                        cacheByEngine: options.sideEffectsCache ? yield getCacheByEngine(ctx.storePath, id) : new Map(),
                        id,
                        inStoreLocation: path.join(ctx.storePath, pkgid_to_filename_1.default(id, options.prefix)),
                        isLocal: false,
                        latest,
                        manifest: pkg,
                        normalizedPref,
            ...
            core_loggers_1.progressLogger.debug({ status: 'error', pkg: options.loggedPkg });
            throw err;
        }
    });
}
                    ...
                    (() => __awaiter(this, void 0, void 0, function* () {
                        ...
                        const fetchedPackage = yield ctx.requestsQueue.add(() => ctx.fetch(opts.resolution, target, {
                            ...
                            onProgress: (downloaded) => {
                                core_loggers_1.progressLogger.debug({ status: 'fetching_progress', pkgId: opts.pkgId, downloaded });
                            },
                            onStart: (size, attempt) => {
                                core_loggers_1.progressLogger.debug({ status: 'fetching_started', pkgId: opts.pkgId, size, attempt });
                            },
                            pkgId: opts.pkgId,
                            prefix: opts.prefix,
                        }), { priority });
