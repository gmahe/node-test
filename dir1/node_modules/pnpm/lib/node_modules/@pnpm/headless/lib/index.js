...
exports.default = (opts) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (!opts.ignoreScripts) {
        for (const importer of opts.importers) {
            ...
            if (scripts.preinstall) {
                yield lifecycle_1.default('preinstall', importer.pkg, scriptsOpts);
            }
        }
    }
    const filterOpts = {
    ...
    const filteredShrinkwrap = filter_shrinkwrap_1.filterByImporters(wantedShrinkwrap, opts.importers.map((importer) => importer.id), Object.assign({}, filterOpts, { failOnMissingDependencies: true }));
    ...
    if (!opts.ignoreScripts) {
        for (const importer of opts.importers) {
            ...
            if (importer.pkg.scripts.install) {
                yield lifecycle_1.default('install', importer.pkg, scriptsOpts);
            }
            if (importer.pkg.scripts.postinstall) {
                yield lifecycle_1.default('postinstall', importer.pkg, scriptsOpts);
            }
            if (importer.pkg.scripts.prepublish) {
                yield lifecycle_1.default('prepublish', importer.pkg, scriptsOpts);
            }
            if (importer.pkg.scripts.prepare) {
                yield lifecycle_1.default('prepare', importer.pkg, scriptsOpts);
            }
        }
    }
    if (reporter) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (shr.packages) {
            ...
            for (const relDepPath of R.keys(shr.packages)) {
                ...
                const pkgLocation = yield opts.storeController.getPackageLocation(pkgId, pkgName, {
                ...
                const modules = path.join(opts.virtualStoreDir, `.${pkgid_to_filename_1.default(depPath, opts.prefix)}`, 'node_modules');
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const alias of R.keys(allDeps)) {
            ...
            if (ctx.graph[childDepPath]) {
                ...
                children[alias] = path.join(ctx.virtualStoreDir, `.${pkgid_to_filename_1.default(childDepPath, ctx.prefix)}`, 'node_modules', pkgName);
            }
